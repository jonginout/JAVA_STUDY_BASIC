...
<script type="text/javascript">
	function detail(no) {
		location.href = "${pageContext.request.contextPath}/board/detail?no="+no;
			...
			<br /> <a href="${pageContext.request.contextPath}/board/writeform">글쓰기</a>
	
			<h3>${page}/ ${pageCount}</h3>
	
			<c:if test="${firstPage!=1}">
				<a
					href="${pageContext.request.contextPath}/board/list?page=${firstPage-1}">
					<button>이전</button>
				</a>
			</c:if>
			...
			<c:if test="${lastPage!=pageCount}">
				<a
					href="${pageContext.request.contextPath}/board/list?page=${lastPage+1}">
					<button>다음</button>
				</a>
			</c:if>
