			...
			<nav>
				<ul class="pagination">
					<c:if test="${pageResult.prev}">
						<li>
							<a href="list3.do?pageNo=${pageResult.beginPage-1}" aria-label="Previous"> 
					...
					<c:forEach var="i" begin="${pageResult.beginPage}" end="${pageResult.endPage}">
						<c:choose>
							...
							<c:otherwise>
								<li>
									<a href="${pageContext.request.contextPath}/board/list3.do?pageNo=${i}">${i}</a>
								</li>						
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:if test="${pageResult.next}">
						<li>
							<a href="list3.do?pageNo=${pageResult.endPage+1}" aria-label="Next">
