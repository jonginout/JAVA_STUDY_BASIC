	 ...
	 method="post" name="modifyForm">
		<table border="1">
			...
			<tr>
				<td>이름</td>
				<td>
					<input type="text" name="staffName" id="staffName" value="${staff.staffName}"/>
				</td>
				<td>주민번호</td>
				<td>
...
<script type="text/javascript">
	...
	var juminNo = '${staff.juminNo}';
	$("#juminNo1").val(juminNo.substring(0,6));
	$("#juminNo2").val(juminNo.substring(7));
	$("select[name=religionCode]").val('${staff.religionCode}');
	$("input:radio[name=schoolCode][value="+'${staff.schoolCode}'+"]").prop("checked", true);
	
	//제이슨
	var skillList = ${staffSkill};
	for (var i = 0; i < skillList.length; i++) {
		$("input:checkbox[name=skillCode][value="+skillList[i].skillCode+"]").prop("checked", true);			
	}
	
	var graduateDay = '${staff.graduateDay}';
	$("select[name=year]").val(graduateDay.substring(0,4));
	$("select[name=month]").val(graduateDay.substring(5,7));
	$("select[name=day]").val(graduateDay.substring(8));
	...
	function clean() {
		location.href = "${pageContext.request.contextPath}/search/modifyform.do?staffNo=${param.staffNo}";
	}
	function search() {
